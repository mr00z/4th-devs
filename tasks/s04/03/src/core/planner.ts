...
export function chooseBuildingInsertionPlan(args: {
  ...
  for (const road of roads) {
    const spawnTiles = getPredictedDismountSpawnTiles(args.map, road)

    if (spawnTiles.length < args.requiredScouts) {
      continue
    }

    const chosenSpawnTiles = spawnTiles.slice(0, args.requiredScouts)
    if (chosenSpawnTiles.some((tile) => !tile.isBuilding)) {
      continue
    }
    ...
    for (const spawnTile of chosenSpawnTiles) {
    ...
    for (const coord of connectedCoverage) {
      const bestDistance = chosenSpawnTiles
    ...
    const directCoverageCount = chosenSpawnTiles.filter((tile) => candidateSet.has(tile.coord)).length
    ...
    plans.push({
      roadCoord: road.coord,
      spawnCoords: chosenSpawnTiles.map((tile) => tile.coord),
