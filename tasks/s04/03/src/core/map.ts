...
export function parseCoord(coord: string): { x: number; y: number } {
  ...
  if (row < 1) {
    throw new Error(`Coordinate row must start from 1: ${coord}`)
  }
...
export function parseMap(rawMap: unknown): GameMap {
  ...
  const tileDefinitions = sourceObj?.tiles && typeof sourceObj.tiles === 'object'
    ? sourceObj.tiles as Record<string, unknown>
    : null
  ...
  const inferTileInfo = (key: string): { symbol: string; isRoad: boolean; isBuilding: boolean; height?: number } | null => {
    const normalizedKey = key.trim().toLowerCase()
    if (!normalizedKey) {
      return null
    }

    const definition = tileDefinitions?.[normalizedKey]
    const definitionRecord = definition && typeof definition === 'object' ? definition as Record<string, unknown> : null
    const symbolFromDefinition = typeof definitionRecord?.symbol === 'string' ? definitionRecord.symbol : undefined

    if (normalizedKey === 'road') {
      return { symbol: symbolFromDefinition ?? 'UL', isRoad: true, isBuilding: false }
    }
    if (normalizedKey === 'tree') {
      return { symbol: symbolFromDefinition ?? 'DR', isRoad: false, isBuilding: false }
    }
    if (normalizedKey === 'empty') {
      return { symbol: symbolFromDefinition ?? '  ', isRoad: false, isBuilding: false }
    }
    if (normalizedKey === 'parking') {
      return { symbol: symbolFromDefinition ?? 'PK', isRoad: false, isBuilding: false }
    }
    if (normalizedKey === 'field') {
      return { symbol: symbolFromDefinition ?? 'BS', isRoad: false, isBuilding: false }
    }
    if (normalizedKey === 'house') {
      return { symbol: symbolFromDefinition ?? 'DM', isRoad: false, isBuilding: true, height: 1 }
    }
    if (normalizedKey === 'church') {
      return { symbol: symbolFromDefinition ?? 'KS', isRoad: false, isBuilding: true, height: 1 }
    }
    if (normalizedKey === 'school') {
      return { symbol: symbolFromDefinition ?? 'SZ', isRoad: false, isBuilding: true, height: 1 }
    }

    const blockMatch = normalizedKey.match(/^block(\d+)$/)
    if (blockMatch) {
      const floors = Number.parseInt(blockMatch[1] || '', 10)
      return {
        symbol: symbolFromDefinition ?? `B${Number.isFinite(floors) ? floors : ''}`,
        isRoad: false,
        isBuilding: true,
        height: Number.isFinite(floors) ? floors : undefined,
      }
    }
  ...
  const applyToken = (tile: Tile, tokenRaw: string): void => {
    ...
    const typeInfo = inferTileInfo(token)
    if (typeInfo) {
  ...
  if (typeof source === 'string') {
    ...
    if (Array.isArray(obj.grid)) {
      for (let y = 0; y < Math.min(obj.grid.length, size); y++) {
  ...
  for (const row of map.tiles) {
    for (const tile of row) {
      ...
      if (tile.isBuilding) {
        buildings.push(tile)
        maxBuildingHeight = Math.max(maxBuildingHeight, tile.height || 0)
      }
    }
  }
  ...
  map.candidates = maxBuildingHeight > 0
    ? buildings.filter((tile) => (tile.height || 0) === maxBuildingHeight)
    : []
