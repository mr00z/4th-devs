...
export const createMcpClient = async (serverName = 'playwright'): Promise<McpClientHandle> => {
  ...
  const client = new Client(
    { name: 's04e01-okoeditor-agent', version: '1.0.0' },
    { capabilities: {} },
  )
...
export const listMcpTools = async (client: Client): Promise<McpToolMeta[]> => {
...
export const callMcpTool = async (client: Client, name: string, args: Record<string, unknown>): Promise<unknown> => {
  ...
  if (isBrowserTool) {
    log.browser(`→ ${name}`, args)
  } else {
    log.info(`Calling MCP tool`, { name, args: Object.keys(args) })
  }
  ...
  if (isBrowserTool) {
    const preview = typeof output === 'string' ? output.slice(0, 300) : JSON.stringify(output).slice(0, 300)
    log.browser(`← ${name}`, { result: preview + (preview.length >= 300 ? '...' : '') })
  }
