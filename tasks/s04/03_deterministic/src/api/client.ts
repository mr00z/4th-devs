...
export async function callHelp(): Promise<VerifyCallResult> {
...
export async function inspectTile(x: number, y: number, unitId?: string): Promise<VerifyCallResult> {
  const answer: VerifyEnvelopeAnswer = { action: 'inspect', x, y }
  if (unitId) {
    answer.unitId = unitId
    answer.object = unitId
  }
  return callVerify(answer)
}
